class CarLockLogger
{
	...
	FileHandle CreateNewLogFile()	{
		string filePath = TRADERPLUS_LOGGER_DIR_SERVER + "CarLock" + "_" + GenerateFullTimeStamp() + ".log";
...
static ref CarLockLogger g_CarLockLogger;
static CarLockLogger GetCarLockLogger(){
	if (!g_CarLockLogger) {
		g_CarLockLogger = new CarLockLogger();
	}

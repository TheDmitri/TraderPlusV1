class TraderPlusMenu extends UIScriptedMenu
{
  ...
  protected vector m_CharacterOrientation;
	...
	{
		if (!localInventoryEntities || !localInventoryEntities.Count())
			return;
	...
	void FillInventoryList(int inventorycount)
	{
		...
		for (int j = 0; j < inventorycount; j++)
		{
			...
			if (m_InventoryGrids[currentGrid] == NULL)
				return;
	...
	override bool OnClick(Widget w, int x, int y, int button)
	{
		...
		switch (w)
		{
		case m_ShowAllCheckBox:
			ShowAllCheckBoxHandler();
			return true;
			break;

		case m_BttnMarket:
			BttnMenuHandler(0, TRADEMODE_BUY);
			return true;
			break;

		case m_BttnInventory:
			BttnMenuHandler(TraderPlusInventoryCategory.SELLABLE, TRADEMODE_SELL);
			return true;
			break;

		case m_BttnLicence:
			BttnMenuHandler(TraderPlusInventoryCategory.LICENCES, TRADEMODE_SELL);
			return true;
			break;

		case m_BttnCar:
			BttnMenuHandler(TraderPlusInventoryCategory.VEHICLES, TRADEMODE_SELL);
			return true;
			break;

		case m_BttnInsurance:
			BttnMenuHandler(TraderPlusInventoryCategory.INSURANCES, TRADEMODE_SELL);
			return true;
			break;

		case m_BttnPlayerPreview:
			SwitchPreview();
			return true;
			break;
		...
		case m_LicenceCloseBttn:
			OnHide();
			return true;
			break;

		case m_InsuranceCheckBox:
			StockListHandler();
			return true;
			break;
		...
		case m_QuantityMultiplier:
			QuantityMultiplierHandler();
			return true;
			break;
	...
	void StockListHandler()
	{
		...
		if (!m_ArticleSelected)
		{
			if (retrievedArticle == NULL && m_CurrentProduct.MaxStock != -1)
			{
				...
				m_ArticleSelected = retrievedArticle;
		}
    ...
    */
		if (Price == -1 || Price == 0 || TradeQty > ItemQty || (MaxStock < stock + m_QtyMultiplier && MaxStock != -1) || (MaxStock == -1 && MaxItemQty != ItemQty && TradeQty < 0))
		{
			if (Price == -1 || Price == 0)
				m_InformationText.SetText("#tpm_cant_trade_item");
			if (TradeQty > ItemQty)
				m_InformationText.SetText("#tpm_tradeqty_over_itemqty");
			if (MaxStock < stock + m_QtyMultiplier && MaxStock != -1)
				m_InformationText.SetText("#tpm_tradeqty_over_stockqty");
			if (MaxStock == -1 && MaxItemQty != ItemQty && TradeQty < 0)
				m_InformationText.SetText("#tpm_tradeqty_over_itemqty");
...
//we define the current product shown so we can transmit it more easely if player wants to sell it
		...
		m_CurrentProduct.Multiplier = m_QtyMultiplier;
		m_CurrentProduct.Quantity = TradeQty;
		if (m_QtyMultiplier > 1 && TradeQty == 0)
			m_CurrentProduct.Quantity = m_QtyMultiplier;
		m_CurrentProduct.Health = health;
		m_CurrentProduct.Price = Price;
		m_CurrentProduct.MaxStock = MaxStock;

class TraderPlusClient
{
			...
			foreach (TraderPlusItem traderPlusItem : ttproducts)
			{
				...
				if (m_TraderStock[category] && m_TraderStock[category][lowClassName] && TraderPlusHelper.CanAddProductToList(filter, traderPlusItem.ClassName))
				{
					for (int h = 0; h < 4; h++)
					{
						...
						{
							if (!m_TraderStock[category][lowClassName].Health[h])
								continue;
	...
	float GetCoefficientState(int state)
	{
		switch (state)
		{
		case TraderPlusItemState.PRISTINE:
			return 1.0;
			break;

		case TraderPlusItemState.WORN:
			return GetTraderPlusConfigClient().CoefficientWorn;
			break;

		case TraderPlusItemState.DAMAGED:
			return GetTraderPlusConfigClient().CoefficientDamaged;
			break;

		case TraderPlusItemState.BADLY_DAMAGED:
			return GetTraderPlusConfigClient().CoefficientBadlyDamaged;
			break;
		...
		foreach (TraderPlusItem tpItem : m_TraderPlusItems.traderPlusItems[category])
		{
			if (CF_String.EqualsIgnoreCase(tpItem.ClassName, classname))
			{
				...
				if (trademode == TRADEMODE_SELL)
				{
					...
					if (maxstock == -1)
						return 1.0 * baseSellprice * qtyMultiplier * GetCoefficientState(state);
					float sellprice = 0;
				...
				else //buy - TRADEMODE_BUY
				{
					...
					if (baseBuyprice == -1)
						return baseBuyprice;
					if (maxstock == -1)
						return 1.0 * baseBuyprice * qtyMultiplier * GetCoefficientState(state);
					;
	...
	{
		name.ToLower();
		if (GetTraderPlusConfigClient().StoreOnlyToPristineState)
			health = 0;
	...
	void GetTraderPlusItemsListFromCategory(string categoryname)
	{
		...
		if (!m_AllTraderPlusItems.traderPlusItems.Find(categoryname, tpItems))
			return;
	...
	void GetTraderPlusItemsListFromAllCategory(array<string> categories, out MapTraderPlusItems tpItemsFromAllCategory)
	{
		...
		for (int k = 0; k < categories.Count(); k++)
		{
			...
			array<ref TraderPlusItem> tpItems = new array<ref TraderPlusItem>;
			if (!m_AllTraderPlusItems.traderPlusItems.Find(categories[k], tpItems))
				continue;
	...
	float CalculateTradeQty(string classname, float tradeqty, int maxqty)
	{
		...
		if (classname.Contains("Mag_") && tradeqty != 0.0)
			return 0;

		//if our tradeqty is a % or maxqty
		if (tradeqty < 1.0 && tradeqty != 0.0 && tradeqty != -1.0)
			return tradeqty * (1.0 * maxqty);

...
{
	if (!GetTraderPlusPriceConfig().EnableAutoCalculation)
		return;
...
{
	if (!GetTraderPlusConfigServer().ConvertTraderConfigToTraderPlus)
		return;
	...
	for (int j = 0; j < m_Trader_ItemsClassnames.Count(); j++)
	{
		...
		if (GetTraderPlusConfigServer().ConvertTraderConfigToTraderPlusWithStockBasedOnCE)
		{
			...
			if (m_Trader_ItemsClassnames[j].Contains("Ammo_"))
				stock = stock / qty;
		}
